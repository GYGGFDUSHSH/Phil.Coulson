...
async def auto_filter(client, message):
    ...
    if 2 < len(message.text) < 100:
        ...
        if imdb and imdb.get('poster'):
            await message.reply_photo(photo=imdb.get('poster'), caption=f"📀 𝖳𝗂𝗍𝗅𝖾: <b>{imdb.get('title')}</b>\n\n🎭 𝖦𝖾𝗇𝗋𝖾: <b>{imdb.get('genres')}</b>\n📆 𝖱𝖾𝗅𝖾𝖺𝗌𝖾: <b>{imdb.get('year')}</b>\n🌟 𝖱𝖺𝗍𝗂𝗇𝗀: <code>{imdb.get('rating')} / 10</code>\n🗳️ 𝖵𝗈𝗍𝖾𝗌</b>: {imdb.get('votes')}\n🌐 𝖢𝗈𝗎𝗇𝗍𝗋𝗒: {imdb.get('country')}\n⏱️ 𝖱𝗎𝗇𝗍𝗂𝗆𝖾: {imdb.get('runtime')}\n🎙 𝖫𝖺𝗇𝗀𝗎𝖺𝗀𝖾: {imdb.get('lang')}\n\n🗒 Storyline: <code>{imdb.get('plot')}</code>", reply_markup=InlineKeyboardMarkup(btn))
        elif imdb:
            await message.reply_photo(photo="https://telegra.ph/file/f9f1b7440984664f37462.jpg", caption=f"🏷 Title: <b>{imdb.get('title')}</b>\n🎭 𝖦𝖾𝗇𝗋𝖾: <b>{imdb.get('genres')}</b>\n📆 𝖱𝖾𝗅𝖾𝖺𝗌𝖾: <b>{imdb.get('year')}</b>\n🌟 𝖱𝖺𝗍𝗂𝗇𝗀: <b>{imdb.get('rating')}</b> / 10\n<b>🗳️ 𝖵𝗈𝗍𝖾𝗌</b>: {imdb.get('votes')}\n<b>🌐 𝖢𝗈𝗎𝗇𝗍𝗋𝗒: {imdb.get('country')}</b>\n<b>⏱️ 𝖱𝗎𝗇𝗍𝗂𝗆𝖾: {imdb.get('runtime')}</b>\n<b>🎙 𝖫𝖺𝗇𝗀𝗎𝖺𝗀𝖾: {imdb.get('lang')}</b>\n\n🗒 Storyline: <code>{imdb.get('plot')}</code>", reply_markup=InlineKeyboardMarkup(btn))
        else:
